let brick = new Brick(new Vector3(-261,98,103), new Vector3(1,1,6), "#678297")
...
// Settings \\
swordDamage = 5 // How much damage does the sword deal when clicked?
swordRange = 6 // How far can the sword hit players?
...
let tool = new Tool("IceDaggerLOL")
...
tool.on("activated", (attacker) => {
    ...
    for (let player of Game.players) {
        if (Game.pointDistance3D(attacker.position, player.position) <= swordRange) {
		    ...
		    lastpos = player.position//local 
                    player.setHealth(player.health - swordDamage) // Damage the player
		    if(player.health > 20){
		    player.centerPrint("You are freezed solid for 4 seconds.");//top
		    }
		    setTimeout(() => {player.setPosition(lastpos) }, 1000);//2}), 1000)//ocl
		    setTimeout(() => {player.setPosition(lastpos)},  1500);//2}), 1000)
		    setTimeout(() => {player.setPosition(lastpos) }, 2000);//2}), 1000)
		    setTimeout(() => {player.setPosition(lastpos) }, 2500);//2}), 1000)
		    setTimeout(() => {player.setPosition(lastpos) }, 3000);//2}), 1000)
		    setTimeout(() => {player.setPosition(lastpos) }, 3500);//2}), 1000)
		    setTimeout(() => {player.setPosition(lastpos) }, 4000);//2}), 1000)
	            attacker.bottomPrint("Victim Health:"+player.health)
		    player.bottomPrint("Attacker Health:}"+attacker.health)//v
		    if(player.health < 20){
		    player.centerPrint("You are freezing to your death");
		    }
...
p.topPrint("You've picked IceDagger sword")
p.equipTool(tool)
